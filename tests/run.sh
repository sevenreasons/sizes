...
"$SIZES" --version >"$OUT"
assert_contains "$OUT" '^sizes 0\.5\.0$' '--version prints current version'
ok '--version'

"$SIZES_WRAPPER" --version >"$OUT"
assert_contains "$OUT" '^sizes 0\.5\.0$' 'root wrapper prints current version'
ok 'root wrapper'
...
sed 's/VERSION="0.5.0"/VERSION="9.9.9"/' "$SIZES" >"$UPGRADE_SOURCE"
...
assert_contains "$OUT" 'sizes: upgraded .+ from 0\.5\.0 to 9\.9\.9' '--upgrade reports old and new version'
...
assert_contains "$OUT" 'current 0\.5\.0, available 9\.9\.9' 'upgrade check reports available version'
ok '--upgrade --check'
...
assert_contains "$OUT" 'from 0\.5\.0 to 9\.9\.9' 'upgrade version installs requested source when override URL is used'
ok '--upgrade --version'
...
env NO_COLOR=1 PATH="$FAKE_BIN:$PATH" "$SIZES" -r --interactive --no-progress "$SAMPLE" >"$OUT" 2>"$ERR"
assert_contains "$OUT" 'sizes — selected extension' 'interactive mode prints selected extension table'
assert_contains "$OUT" 'Top files for' 'interactive mode prints top-files preview after selection'
ok '--interactive'
