...
"$SIZES" --version >"$OUT"
assert_contains "$OUT" '^sizes 0\.7\.8$' '--version prints current version'
ok '--version'

"$SIZES_WRAPPER" --version >"$OUT"
assert_contains "$OUT" '^sizes 0\.7\.8$' 'root wrapper prints current version'
ok 'root wrapper'
...
sed 's/VERSION="0.7.8"/VERSION="9.9.9"/' "$SIZES" >"$UPGRADE_SOURCE"
...
assert_contains "$OUT" 'sizes: upgraded .+ from 0\.7\.8 to 9\.9\.9' '--upgrade reports old and new version'
...
assert_contains "$OUT" 'current 0\.7\.8, available 9\.9\.9' 'upgrade check reports available version'
ok '--upgrade --check'
...
assert_contains "$OUT" 'from 0\.7\.8 to 9\.9\.9' 'upgrade version installs requested source when override URL is used'
ok '--upgrade --version'
...
if grep -q 'kitty +kitten icat\|wezterm imgcat\|imgcat "$human_path"\|viu -w' "$SIZES"; then
