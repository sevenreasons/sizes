...
_sizes_completion() {
    ...
    if [[ "$cur" == -* ]]; then
        COMPREPLY=( $(compgen -W '-r --recursive --depth --follow -x --one-file-system --max-files -n --limit --min-size --min-share -e --exact -E --errors --include --exclude --type --top-files --top-dirs --by-dir -i --interactive --interactive-no-preview --allow-delete --sort --format --save --group-by --plain --no-progress --no-color --upgrade --check --version -h --help' -- "$cur") )
        return 0
    fi
